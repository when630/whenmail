...
- **후속 리마인더** — 초안을 만들면서 "N일 뒤 회신 없으면 알리기"를 체크하거나, 사람 상세에서 날짜를 직접 지정한다. 회신이 오면 자동으로 닫히고, 할 일에서 미루거나 완료할 수 있다
- **할 일** — 활동 화면의 탭. 걸어 둔 후속과 답장 대기를 기한 순으로 모아 보고, 거기서 바로 초안을 만든다
- **템플릿 시퀀스** — 인사 → 1주 후 자료 → 2주 후 미팅 제안처럼 템플릿을 순서로 묶는다. 시작하면 1단계가 할 일로 뜨고 초안을 만들면 다음 단계가 예약된다. **자동으로 보내지 않는다**
...
- 스키마는 `schema_version`으로 관리한다. v1(명함 1장 = 연락처 1건) → v2(사람/회사/활동) → v3(계정) → v4(메일 인덱스·알림함) → v5(후속·시퀀스)까지 첫 실행 시 자동 이전되며, v1에서 올라올 때는 `whenmail.db.bak-v1` 사본을 남긴다
...
- [설계 문서](docs/01_설계.md) — 요구사항, IA, UX 플로우, 데이터 모델, 계정·어댑터 전략, 마일스톤 M1~M7(전체 완료), 오픈이슈
