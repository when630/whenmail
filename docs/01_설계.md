...
### D-12. 후속은 규칙과 건별을 나눠 둔다 ✅
- **전역 규칙**(M6): "보낸 뒤 N일 답 없음"을 사람 전체에 적용해 답장 대기를 잡는다. 설정 하나로 끝
- **건별 후속**(M7, `follow_up`): "이 사람은 언제까지 뭘 챙길지"를 직접 건다. 초안 만들 때 체크 한 번, 또는 사람 상세에서 날짜 지정
- 둘이 겹치지 않게: **열린 후속이 있는 사람은 전역 답장 대기 알림에서 제외**한다. 명시적으로 건 것이 규칙보다 우선
- 한 사람에게 후속이 여러 건 걸리는 것은 정상이다(직접 건 것 + 시퀀스 단계). 할 일에 각각 뜬다
- 회신이 오면 `auto_close_on_reply`인 후속을 자동으로 닫는다. 끄면 회신과 무관하게 남는다

### D-13. 시퀀스는 순서만 기억한다 ✅
- `sequence` = 이름 + 단계 목록(템플릿, 앞 단계 뒤 며칠). `person_sequence` = 누구에게 몇 단계까지 진행했는지
- 시작하면 1단계가 **바로 할 일**로 뜬다(지연 0). 초안을 만들면 그 단계가 완료되고 다음 단계가 `delay_days` 뒤로 예약된다
- **자동 전송은 하지 않는다.** 때가 되면 알림만 띄우고, 보내는 것은 항상 사람이 확인하고 직접 한다 (D-1 유지)
- 중단하면 남은 단계의 후속이 취소된다. 다시 시작하면 1단계부터
- 시퀀스를 삭제하면 그 시퀀스의 후속도 함께 사라진다(`ON DELETE CASCADE`)
...
| **M7 후속·시퀀스** ✅ *(v0.8.0)* | "초안 만들면서 '2주 후 답 없으면 알려줘'를 걸고, 회신 오면 자동으로 사라진다" | 후속 리마인더, 회신 감지 자동 해제, 활동 뷰의 할 일 탭, 템플릿 시퀀스(순서만, 자동 전송 없음) | M6 |
...
- v2.5 (2026-09-17): **M7 구현 완료 — 마일스톤 전체 완료.** 스키마 v5(follow_up, sequence/sequence_step/person_sequence, notification.follow_up_id). 건별 후속 리마인더(초안 만들 때 체크 또는 사람 상세에서 날짜 지정, 미루기·완료), 회신 도착 시 자동 종료, 활동 화면의 할 일 탭(후속 + 답장 대기 합쳐 기한 순), 템플릿 시퀀스(순서만·자동 전송 없음, 단계 완료 시 다음 단계 예약), 알림함에 후속 종류 추가와 "초안 만들기" 액션. 결정 D-12·D-13 추가. v0.8.0 릴리즈
