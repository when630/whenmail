...
| **M5 계정 등록 + 어댑터** | "회사 M365와 개인 Gmail을 등록하고, 같은 템플릿을 사람에 따라 다른 계정으로 초안 만든다" | 계정 CRUD·safeStorage, 로컬 Outlook 계정 자동 변환, Graph 초안 어댑터(오픈이슈 #1 해소 경로), Gmail 초안 어댑터, IMAP APPEND 어댑터, 계정 선택 단계, 어댑터 지원 범위 배지 | Azure 앱 등록(#5), Google OAuth 클라이언트(#6) |
| **M6 읽기 통합** | "사람 상세를 열면 그 사람과 오간 메일이 계정 구분 없이 시간순으로 보이고, 답 없는 사람 목록이 뜬다" | ReadAdapter(Graph·Gmail·IMAP), mail_index, 동기화 스케줄러, 마지막 연락일·답장 대기 파생, 원문 열기 링크 | 읽기 스코프 승인(#5, #6) |
...
| # | 이슈 | 상태 |
|---|---|---|
| 1 | 신형 Outlook 단독 환경의 HTML 초안 품질(.eml 불안정) → **M5의 Microsoft 365(Graph) 계정이 해소 경로**. 신형 Outlook 데스크톱이 Graph로 만든 초안을 초안 폴더에 바로 보여주는지 실기기 검증 | M5에서 검증 |
...
| 5 | Azure 앱 등록 방식: 개인(when630) 테넌트에 멀티테넌트 공개 클라이언트로 등록 vs 회사 테넌트 등록. 회사 테넌트 정책이 서드파티 앱 동의를 막으면 관리자 동의 필요 *(추정)*. `Mail.ReadWrite`(초안)와 `Mail.Read`(읽기)를 단계적으로 요청 | M5 전 확인 |
| 6 | Gmail OAuth: `gmail.compose`는 민감, `gmail.metadata`/`gmail.readonly`는 제한 스코프로 배포 시 Google 검증 필요. 개인용은 **테스트 모드(테스트 사용자 등록)**로 운영하는 것으로 시작하고, 배포 여부가 생기면 재검토 | M5 결정 |
| 7 | 내보내기 vCard(.vcf) 지원 여부 — 메일 클라이언트 주소록 가져오기용. CSV/xlsx 먼저, 수요 확인 후 | M4 이후 |
| 8 | 읽기 동기화의 상주 여부: 앱 실행 중에만 vs 트레이 상주·시작 시 실행. 후속 리마인더 알림이 상주를 요구할 수 있음 | M6 |
...
| 11 | 자격증명 파일이 백업에서 빠지므로 복원 후 재인증 UX. 계정 목록은 복원되되 "재연결 필요" 상태로 표시 | M5 |
